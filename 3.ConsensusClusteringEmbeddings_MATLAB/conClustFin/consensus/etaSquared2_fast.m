function [varargout] = etaSquared2_fast(inData1,inData2,varargin)
% -------------------------------------------------------------------------
...
c = combvec(inData1',inData2');
mbar = mean(c,1);

% this is completely vectorized but runs slower
% m = (c(1:size(c,1)/2,:)+c(((size(c,1)/2) + 1):size(c,1),:))./2;
% n = ((c(1:size(c,1)/2,:) - m).^2) + ((c(((size(c,1)/2) + 1):size(c,1),:) - m).^2);
% d = ((c(1:size(c,1)/2,:) - mbar).^2) + ((c(((size(c,1)/2) + 1):size(c,1),:) - mbar).^2);
% eta2 = reshape((1 - sum(n,1)./sum(d,1)),[size(inData1,1),size(inData2,1)]);

t1 = c(1:size(c,1)/2,:);
t2 = c(((size(c,1)/2) + 1):size(c,1),:);
m = (t1+t2)./2;
n = ((t1 - m).^2) + ((t2 - m).^2);
d = ((t1 - mbar).^2) + (t2.^2);
eta2 = reshape((1 - sum(n,1)./sum(d,1)),[size(inData1,1),size(inData2,1)]);
